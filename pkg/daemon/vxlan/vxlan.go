...
func ensureTCRules(link *netlink.Vxlan) error {
	...
	runCmd := exec.Cmd{
		...
		Args: append([]string{tcPath},
			// "tc filter replace dev eth0.vxlan4 parent 8004: handle 800::800 prio 1 u32 match
			// u16 0x01 0x01 at -14 action pedit ex munge eth dst set ff:ff:ff:ff:ff:ff"
			"filter", "replace", "dev", link.Name, "parent", netlink.HandleStr(rootQdisc.Attrs().Handle),
			"handle", "800::800", "prio", "1", "u32", "match", "u16", "0x01", "0x01", "at", "-14",
